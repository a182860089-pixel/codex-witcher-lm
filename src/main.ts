  ...
  profiles: [
    {
      id: "grok",
      display_name: "grok",
      base_url: "https://kedaya.ai/v1",
      models: [
        previewModel("grok-4.6", "grok-4.6", "日常开发与复杂任务"),
        previewModel("grok-4", "grok-4", "快速编码与迭代"),
      ],
...
const previewProxy: LocalProxyStatus = {
  enabled: true,
  running: true,
  recoveryRequired: false,
  manualRecoveryRequired: false,
  currentProfileId: "grok",
...
const PREVIEW_HINT = "预览模式：演示数据，不会改动已安装的桌面端。";
const nativeAvailable = false;
let proxyApiAvailable = true;
let localProxy = previewProxy;
let codexAccount = unavailableAccountStatus;
let codexAccountAvailable = true;
...
app.innerHTML = `
  <div class="preview-root">
    <div class="preview-banner" role="status">预览模式 · 演示数据 · 不影响已安装桌面端</div>
    <div class="app-shell">
    <aside class="sidebar" aria-label="应用导航">
      ...
      <div class="sidebar-footer">
        ...
        <span class="preview-chip">预览 · 演示数据</span>
    ...
    <div class="workspace">
      <header class="app-header">
        <div class="page-heading">
          ...
          <span id="page-description">选择接入和模型，应用会自动完成切换。</span>
        </div>
...
document.addEventListener("pointerdown", (event) => {
  ...
  if (target instanceof HTMLElement) {
    const trigger = target.closest("button, .nav-item, .profile-card");
...
async function refreshDashboard(): Promise<void> {
  if (!nativeAvailable) {
    renderDashboard();
    setStatus("预览模式已载入演示数据。切换和保存不会改动已安装桌面端。", "info");
    return;
  }
...
async function activateOfficial(): Promise<void> {
  ...
  await run("正在切换到 OpenAI 官方路由…", async () => {
    if (!nativeAvailable) throw new Error(PREVIEW_HINT);
    try {
...
function openEditor(): void {
  ...
  void animateDialogIn(editor, lastDialogTrigger);
...
function maybeShowRestartNotice(): void {
  ...
  if (!dialog.open) {
    dialog.showModal();
    void animateDialogIn(dialog, lastDialogTrigger);
  }
}
